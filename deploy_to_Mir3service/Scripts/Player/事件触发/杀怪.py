...
def OnKillMon(args):
	...
	# EVE20260908_WEAPON_EXP
	# If weapon is Refinable, licensed GainExperience skips amount/10; top up here.
	try:
		wpn = Sender.Equipment[int(EquipmentSlot.Weapon)]
		if wpn is not None:
			from Library import UserItemFlags
			if (wpn.Flags & UserItemFlags.Refinable) == UserItemFlags.Refinable:
				wpn.Experience += (MonsterInfo.Experience / 10)
	except Exception:
		pass
	...
	if(Sender.GroupMembers):                    # 队伍判断
		...
		if(PlayerGetV(Sender.GroupMembers[0],GV_KILLMON_ZMGWCOUNT) == 8): # ！！数字对应着小boss数量
			PlayerSetV(Sender.GroupMembers[0],GV_KILLMON_ZMGWCOUNT,0)   #先把杀死诺玛队长的计数清零
			#在玩家当前地图刷出指定的怪物(刷怪的X和Y坐标 5格范围内 刷怪的名字  刷怪的数量)
	...
	if(Sender.GroupMembers):                    # 队伍判断
		if MonsterInfo.Index in KILL_PKS_ELITE:  #判断击杀诺玛队长的信息
			PlayerSetV(Sender.GroupMembers[0],GV_KILLMON_PKSCOUNT,PlayerGetV(Sender.GroupMembers[0],GV_KILLMON_PKSCOUNT) + 1)   #杀死小BOSS变量增加
			Sender.Connection.ReceiveChat("击杀次数{}".format(PlayerGetV(Sender.GroupMembers[0],GV_KILLMON_PKSCOUNT)), MessageType.System)

		if(PlayerGetV(Sender.GroupMembers[0],GV_KILLMON_PKSCOUNT)== 4):  #如果杀死小BOSS的数量达到4只
			Sender.Connection.ReceiveChat("触发boss, 总击杀次数{}".format(PlayerGetV(Sender.GroupMembers[0],GV_KILLMON_PKSCOUNT)), MessageType.System)
			PlayerSetV(Sender.GroupMembers[0],GV_KILLMON_PKSCOUNT,0)   #先把杀死小BOSS的计数清零
	...
	# 奇遇地图怪物脚本
        if (list) in monList1:
        	Sender.Connection.ReceiveChat("你宰杀了隐藏大佬{}，你将获得无尽宝藏还是无尽厄运……".format(MonsterInfo.MonsterName), MessageType.System)
        	select = random.randint(0,5000)
        	if select < 1:
        			Sender.ItemStatsChangeRefresh(int(EquipmentSlot.Weapon), Stat.Luck, 1, StatSource.Enhancement)   #给手里武器1点幸运属性并刷新属性值
        			Sender.Connection.ReceiveChat("天降机缘，你的武器增加了1点【幸运】", MessageType.System)
        	elif select < 2:
        			Sender.ItemStatsChangeRefresh(int(EquipmentSlot.Weapon), Stat.CriticalChance, 1, StatSource.Enhancement)   #给手里武器增加1%暴击几率属性并刷新属性值
        			Sender.Connection.ReceiveChat("天降机缘，你的武器增加了1点【暴击】", MessageType.System)
        	elif select < 3:
        			Sender.ItemStatsChangeRefresh(int(EquipmentSlot.Weapon), Stat.DropRate, 1, StatSource.Enhancement)   #给手里武器增加1%爆率几率属性并刷新属性值
        			Sender.Connection.ReceiveChat("天降机缘，你的武器增加了1点【爆率】", MessageType.System)
        	elif select < 5:
        			Sender.GiveItem("新能源（小）",1)         #给道具+数量
        			Sender.Connection.ReceiveChat("别杀我，给你我收藏的宝物【新能源】", MessageType.System)
        	elif select < 6:
        			Sender.GiveItem("虚空宝珠（衣服）",1)         #给道具+数量
        			Sender.Connection.ReceiveChat("别杀我，给你我收藏的宝物【虚空宝珠（衣服）】", MessageType.System)
        	elif select < 6:
        			Sender.GiveItem("虚空宝珠（鞋子）",1)         #给道具+数量
        			Sender.Connection.ReceiveChat("别杀我，给你我收藏的宝物【虚空宝珠（鞋子）】", MessageType.System)
        	elif select < 6:
        			Sender.GiveItem("虚空宝珠（头盔）",1)         #给道具+数量
        			Sender.Connection.ReceiveChat("别杀我，给你我收藏的宝物【虚空宝珠（头盔）】", MessageType.System)
        	elif select < 6:
        			Sender.GiveItem("虚空宝珠（戒指）",1)         #给道具+数量
        			Sender.Connection.ReceiveChat("别杀我，给你我收藏的宝物【虚空宝珠（戒指）】", MessageType.System)
        	elif select < 7:
        			Sender.GiveItem("虚空宝珠（项链）",1)         #给道具+数量
        			Sender.Connection.ReceiveChat("别杀我，给你我收藏的宝物【虚空宝珠（项链）】", MessageType.System)
        	elif select < 7:
        			Sender.GiveItem("虚空宝珠（手镯）",1)         #给道具+数量
        			Sender.Connection.ReceiveChat("别杀我，给你我收藏的宝物【虚空宝珠（手镯）】", MessageType.System)
        	elif select < 8:
        			Sender.GiveItem("经验爆率补药",1)         #给道具+数量
        			Sender.Connection.ReceiveChat("别杀我，给你我收藏的宝物【经验爆率补药】", MessageType.System)
        	elif select < 10:
        			Sender.TeleportByMapIndex(1503,33,38)	#飞地图ID X坐标 Y坐标
        			Sender.Connection.ReceiveChat("触发了咒语：移动巴拉巴拉写什么", MessageType.System)
        	elif select < 11:
        			Sender.TeleportByMapIndex(1526,15,26)	#飞地图ID X坐标 Y坐标
        			Sender.Connection.ReceiveChat("触发了咒语：移动巴拉巴拉写什么", MessageType.System)
        	elif select < 15:
        			Sender.TeleportByMapIndex(2,40,33)	#飞地图ID X坐标 Y坐标
        			Sender.Connection.ReceiveChat("触发了咒语：移动巴拉巴拉写什么", MessageType.System)
        	elif select < 20:
        			Sender.GiveItem("高级技能残片",1)         #给道具+数量
        			Sender.Connection.ReceiveChat("别杀我，给你我收藏的宝物【高级技能残片】", MessageType.System)

        	elif select < 25:
        			current_map.CreateMon(77,82,2,'红拂', 1)  #在指定地图刷的怪物 （怪物名字或者ID，怪物数量，刷怪坐标X,Y,范围,)
        			Sender.Connection.ReceiveChat("玩家 {} 在破碎虚空冒险，大地颤抖，“红拂”出现在 {} [{}:{}]".format(Sender.Name, Sender.CurrentMap.Info.Description, Sender.CurrentLocation.X, Sender.CurrentLocation.Y), MessageType.System)

        	elif select < 500:
        			GiveGameGold(Sender,200)         #给元宝+数量
        			Sender.Connection.ReceiveChat("别杀我，给你【200元宝】", MessageType.System)
        	elif select < 500:
        			GivePrestige(Sender,200)         #给元宝+数量
        			Sender.Connection.ReceiveChat("别杀我，给你【200声望】", MessageType.System)
        	elif select < 1000:
        			GiveGameGold(Sender,100)         #给元宝+数量
        			Sender.Connection.ReceiveChat("别打我，给你【100元宝】", MessageType.System)
        	elif select < 1000:
        			GivePrestige(Sender,50)         #给声望+数量
        			Sender.Connection.ReceiveChat("别杀我，给你【50点声望】", MessageType.System)
