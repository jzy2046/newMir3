...
def GiveGameGold(Sender,Count):   #给予元宝/赞助币（与客户端显示1:1，内部存储*100）
	Sender.GameGold += Count * 100
	Sender.GameGoldChanged()

def SubGameGold(Sender,Count):    #扣除元宝/赞助币（与客户端显示1:1，内部存储*100）
	need = Count * 100
	if (Sender.GameGold < need):
		return
	Sender.GameGold -= need
	Sender.GameGoldChanged()

def HasGameGold(Sender,Count):    #判断赞助币是否足够（显示单位）
	return Sender.GameGold >= Count * 100
