...
diy_dat = Path(r"E:\Mir3_16000\Data\Diy\Mir3.dat")
assert latest_db.is_file() and sys_db.is_file() and diy_dat.is_file()
_diy_txt = diy_dat.read_text(encoding="utf-8", errors="replace")
assert "ComfortValue=False" in _diy_txt and "ComfortValue=True" not in _diy_txt
...
out = ROOT / "patches_ready_20260906" / "PC_upload_overwrite_v15_x64_comfortvalue"
zip_out = ROOT / "patches_ready_20260906" / "PC_upload_v15_x64_comfortvalue.zip"
out.mkdir(parents=True, exist_ok=True)
for f in list(out.iterdir()):
    if f.is_file():
        f.unlink()

tmp = ROOT / "tools" / "Mir3ComfortFix" / "x64_exe_v15"
...
clen_diy = gzip_file(diy_dat, out / "Data-Diy-Mir3.dat.gz")
csum_exe = md5_file(tmp / "Mir3.exe")
csum_db = md5_file(latest_db)
csum_diy = md5_file(diy_dat)
assert len(raw_exe) == 4130304
assert 2161000 <= clen_mir3 <= 2161300, clen_mir3
...
readme = [
    f"PC 20260909 x64+ComfortValue pack v15 stamp={stamp}",
    ...
    f"Diy Mir3.dat size={diy_dat.stat().st_size} md5={csum_diy.hex()} gz={clen_diy}",
