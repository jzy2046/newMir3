    ...
    public sealed class GameScene : DXScene
    {
        ...
        public UserObject User
        {
            ...
            set
            {
                ...
                HookHelper.LoadPickFilterConfig(value);   //20250324
        ...
        {
            // 左侧功能区布局
            NPCBox.Location = new Point(NativeUI.ScreenOffset.X, 0); // NPC对话框紧贴左侧边缘

            LeftUpUserPanel.Location = new Point(10, 0); // 用户信息面板，左侧顶部，保留10像素边距
         
            // 主功能按钮组，位于用户信息面板下方10像素处
            LeftCentButtonsPanel.Location = new Point(
                0 + NativeUI.ScreenOffset.X,
                LeftUpUserPanel.Location.Y + LeftUpUserPanel.Size.Height - 650);

            // 用户功能菜单（点击头像弹出的菜单），位于头像下方100像素处
            UserFunctionControl.Location = new Point(
                LeftUpUserPanel.Location.X + 15,
                LeftUpUserPanel.Location.Y + 100);

            // 敌方信息面板，右上角位置
            EnemyMemberBox.Location = new Point(
                Size.Width - EnemyMemberBox.Size.Width - 24 - NativeUI.ScreenOffset.Y,
                20);

            // 右侧技能栏，底部上方
            MagicBarBox.Location = new Point(
                Size.Width - MagicBarBox.Size.Width - NativeUI.ScreenOffset.Y,
                Size.Height - MagicBarBox.Size.Height - ExpPanel.Size.Height);

            // 物品快捷栏，位于技能栏上方
            BeltBox.Location = new Point(
                 MagicBarBox.Location.X - BeltBox.Size.Width + 94,//xxk039  原来是10
                 ExpPanel.Location.Y - BeltBox.Size.Height - 100);

            // BUFF状态图标，用户信息面板右侧
            BuffBox.Location = new Point(
                LeftUpUserPanel.Location.X + LeftUpUserPanel.Size.Width - 420,
                LeftUpUserPanel.Location.Y + 25);

            //血槽
            MainPanel.Location = new System.Drawing.Point(
                 MagicBarBox.Location.X - MainPanel.Size.Width - 109,//xxk039  原来是10
                 ExpPanel.Location.Y - MainPanel.Size.Height + 80);

            // 底部中央面板，位于经验条上方
            DownCentPanel.Location = new Point(
                LeftUpUserPanel.Location.X + LeftUpUserPanel.Size.Width - 240,
                 LeftUpUserPanel.Location.Y + 25);
            // 左侧聊天框，底部向上50像素
            ChatRBox.Location = new Point(
               BeltBox.Location.X - ChatRBox.Size.Width - 2222,  // 水平对齐：与面板左边缘对齐
                ExpPanel.Location.Y - ChatRBox.Size.Height - 2850); // 垂直定位：面板下沿坐标向上50像素
                                                                     //NativeUI.ScreenOffset.X == 0 ? 10 : NativeUI.ScreenOffset.X,
                                                                     //Size.Height - ChatLBox.Size.Height - 50);
           // 队友信息面板，位于主功能按钮右侧 
            GroupMemberBox.Location = new Point(
                LeftCentButtonsPanel.Location.X + LeftCentButtonsPanel.Size.Width,
                LeftCentButtonsPanel.Location.Y);

            // 虚拟摇杆控件，底部左侧
            VirtualWalkStick.Location = new Point(
                  60,  // 紧贴窗体左边缘（无间距）
                  ExpPanel.Location.Y - VirtualWalkStick.Size.Height - 30  // 经验条顶部向上偏移10px
            );


            // 小地图，右上角位置
            MiniMapBox.Location = new Point(Size.Width - MiniMapBox.Size.Width - 30, 0);// 以前是width - 50  qq715590

            // 特殊状态标签，位于聊天框上方异界之门黄字
            NetherworldGateLabel.Location = new Point(
                  LeftUpUserPanel.Location.X + LeftUpUserPanel.Size.Width - 620,
                LeftUpUserPanel.Location.Y + 5);
        }
        /// <summary>
        /// 设置窗体默认显示的位置
        /// </summary>
        private void SetDefaultLocations()
        {
            ...
            ChangeLandscape();
            ChatLBox.Location = new Point(
                 //(Size.Width - ChatRBox.Size.Width) / 2 - 50,
                 // DownCentPanel.Location.Y - ChatRBox.Size.Height - 10);
                MagicBarBox.Location.X - ChatLBox.Size.Width + 120,  // 水平对齐：与面板左边缘对齐
                ExpPanel.Location.Y - ChatLBox.Size.Height + 20);
        ...
        public override void Process()
        {
            ...
            else if (!Config.SmoothRendering)  //20250425添加 if (!Config.SmoothRendering)原单else
            {
                MoveFrame = false;
            }
        ...
        public void CreateItemLabel()
        {
            ...
            {
                BackColour = Color.FromArgb(248, 22, 14, 10),   //深色木纹底（无独立边框素材时）
                Border = true,
                BorderColour = Color.FromArgb(190, 140, 85),     //暖色描边
                DrawTexture = true,
                IsControl = false,
                IsVisible = true,
            };
            ...
            // 物品图标（左上角），使用 Inventory 库与 ItemInfo.Image（与拖拽/装备格一致）
            DXImageControl itemIcon = null;
            int headerLeft = 4;
            int headerTop = 4;
            if (displayInfo.Image >= 0)
            {
                itemIcon = new DXImageControl
                {
                    LibraryFile = LibraryFile.Inventory,
                    Index = displayInfo.Image,
                    Parent = ItemLabel,
                    IsControl = false,
                    Location = new Point(4, 4),
                };
                // 限制过大图标占用提示宽度
                if (itemIcon.Size.Width > 42 || itemIcon.Size.Height > 42)
                {
                    itemIcon.FixedSize = true;
                    itemIcon.Size = new Size(Math.Min(itemIcon.Size.Width, 42), Math.Min(itemIcon.Size.Height, 42));
                }
                headerLeft = itemIcon.DisplayArea.Right + 6;
            }
            ...
            {
                ForeColour = Color.Gold,   //道具字体颜色
                Location = new Point(headerLeft, headerTop + 1),
                Parent = ItemLabel,
                Text = "▼",
            ...
            if (!string.IsNullOrEmpty(MouseItem.CustomPrefixText))
            {
                ...
                {
                    ForeColour = MouseItem.CustomPrefixColor == Color.Empty ? Color.Gold : MouseItem.CustomPrefixColor,
                    Location = new Point(itemNameOffset - 2, headerTop),
                    Parent = ItemLabel,
            ...
            {
                ForeColour = Color.Gold,   //道具名字金色
                Location = new Point(itemNameOffset, headerTop),
                Parent = ItemLabel,
            ...
            // 类型标签（右上，如 [靴子]）
            string typeTagText = $"[{displayInfo.ItemType.Lang()}]";
            DXLabel typeTag = new DXLabel
            {
                ForeColour = Color.FromArgb(210, 195, 150),
                Parent = ItemLabel,
                Text = typeTagText,
                IsControl = false,
            };

            int nameBottom = label.DisplayArea.Bottom;
            int iconBottom = itemIcon != null ? itemIcon.DisplayArea.Bottom : nameBottom;
            int rowBottom = Math.Max(nameBottom, iconBottom);
            int minWidth = Math.Max(label.DisplayArea.Right + typeTag.Size.Width + 12, itemIcon != null ? itemIcon.DisplayArea.Right + 8 : 0);
            ItemLabel.Size = new Size(Math.Max(minWidth, 160), rowBottom + 4);
            typeTag.Location = new Point(Math.Max(ItemLabel.Size.Width - typeTag.Size.Width - 4, label.DisplayArea.Right + 8), headerTop);
            ItemLabel.Size = new Size(
                Math.Max(ItemLabel.Size.Width, typeTag.DisplayArea.Right + 4),
                Math.Max(ItemLabel.Size.Height, typeTag.DisplayArea.Bottom + 4));
            ...
            if ((MouseItem.Flags & UserItemFlags.Bound) == UserItemFlags.Bound)
            {
                label = new DXLabel
                {
                    ForeColour = Color.LimeGreen, // 绑定/特殊：绿色
                    Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                    Parent = ItemLabel,
                    Text = "绑定物品".Lang(),
                };
            ...
            if (!string.IsNullOrEmpty(displayInfo.Description))   //道具说明
            {
                ...
                label = new DXLabel
                {
                    ForeColour = Color.FromArgb(220, 220, 200),
                    Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                    Parent = ItemLabel,
            ...
            CreateSetItemInfo(true); // 默认显示套装部件与加成，无需按Alt
            #endregion
        ...
        private void EquipmentItemInfo(string prefix = "")
        {
            ...
            int TipGroup(Stat s)
            {
                // 0 = basic/non-special (top), 1 = special whitelist (bottom)
                if (IsTipSpecialStat(s)) return 1;
                return 0;
            }
            ...
            bool WouldDrawFullItemStat(FullItemStat fullItemStat, HashSet<Stat> drawn)
            {
                ...
                if (Globals.GemList.Contains(fullItemStat.StatSource))
                    return false;

                if (drawn.Contains(fullItemStat.Stat))
                    return false;
            ...
            bool WouldDrawPairStat(KeyValuePair<Stat, int> pair)
            {
                // Skip tip lines for AttackSpeed/Accuracy when value is 0 (incl. 0(0))
            ...
            void DrawFullItemStat(FullItemStat fullItemStat)
            {
                ...
                if (isOther)
                {
                    label.ForeColour = Color.MediumPurple; // 淬炼/精炼：紫色
                }
                ItemLabel.Size = new Size(label.DisplayArea.Right + 4 > ItemLabel.Size.Width ? label.DisplayArea.Right + 4 : ItemLabel.Size.Width,
                    label.DisplayArea.Bottom > ItemLabel.Size.Height ? label.DisplayArea.Bottom : ItemLabel.Size.Height);
            }
            ...
            {
                bool drewBasic = false;
                foreach (FullItemStat fullItemStat in MouseItem.FullItemStats)
                {
                    if (TipGroup(fullItemStat.Stat) != 0) continue;
                    if (!WouldDrawFullItemStat(fullItemStat, statsDrawn)) continue;
                    DrawFullItemStat(fullItemStat);
                    drewBasic = true;
                }
                ...
                foreach (FullItemStat fullItemStat in MouseItem.FullItemStats)
                {
                    if (TipGroup(fullItemStat.Stat) != 1) continue;
                    if (WouldDrawFullItemStat(fullItemStat, statsDrawn))
                    {
                        hasBottom = true;
                        break;
                    }
            ...
            else
            {
                ...
                {
                    if (TipGroup(pair.Key) != 0) continue;
                    if (!WouldDrawPairStat(pair)) continue;
                    DrawPairStat(pair);
                    drewBasic = true;
                }
                ...
                foreach (KeyValuePair<Stat, int> pair in stats.Values)
                {
                    if (TipGroup(pair.Key) != 1) continue;
                    if (WouldDrawPairStat(pair))
                    {
                        hasBottom = true;
                        break;
                    }
            ...
            switch (displayInfo.ItemType)
            {
                case ItemType.Weapon:
                    ...
                    // 仍显示武器等级/修炼（新版升级开启时 tip 也要能看到当前等级）
                    // if (CEnvir.ClientControl.NewWeaponUpgradeCheck) break;

                    label = new DXLabel
                    {
                        ForeColour = Color.White,
                        Location = new Point(4, ItemLabel.Size.Height),
                        Parent = ItemLabel,
        ...
        private void CreateMagicLabel()
        {
            if (MouseMagic != null)
            {
                ...
                {
                    BackColour = Color.FromArgb(Convert.ToInt32(178.5), 167, 158, 93),
                    DrawTexture = true,
                    IsControl = false,
                    IsVisible = true
                };
                DXLabel dXLabel = new DXLabel();
                dXLabel.ForeColour = Color.FromArgb(255, 192, 121);
                dXLabel.Location = new Point(0, 10);
                dXLabel.Parent = MagicLabel;
                dXLabel.Font = new Font(Config.FontName, CEnvir.FontSize(9f), FontStyle.Bold);
                dXLabel.Text = "［" + MouseMagic.Lang((MagicInfo p) => p.Name) + "］";
                DXLabel label = dXLabel;
                MagicLabel.Size = new Size(label.DisplayArea.Right + 4, label.DisplayArea.Bottom + 4);
                string text = "";
                if (User.Magics.TryGetValue(MouseMagic, out var magic))
                {
                    text = magic.Level switch
                    {
                        ...
                        _ => $"{magic.Experience}/{(magic.Level - 2) * Config.SkillExpDrop}",
                    };
                }
                label = new DXLabel
                {
                    ForeColour = Color.FromArgb(255, 192, 121),
                    Parent = MagicLabel,
                    Font = new Font(Config.FontName, CEnvir.FontSize(9f), FontStyle.Bold),
                    Location = new Point(label.DisplayArea.Right + 80, 10),
                    Text = "〈" + MouseMagic.Action.Lang() + "〉"
                };
                ...
                label = new DXLabel
                {
                    ForeColour = Color.FromArgb(255, 255, 240),
                    Location = new Point(0, MagicLabel.DisplayArea.Bottom + 8),
                    Parent = MagicLabel,
                    Text = "1级技能需要等级".Lang() + $"：{MouseMagic.NeedLevel1}"
                };
                MagicLabel.Size = new Size((label.DisplayArea.Right + 4 > MagicLabel.Size.Width) ? (label.DisplayArea.Right + 4) : MagicLabel.Size.Width, label.DisplayArea.Bottom);
                label = new DXLabel
                {
                    ForeColour = Color.FromArgb(255, 255, 240),
                    Location = new Point(0, label.DisplayArea.Bottom),
                    Parent = MagicLabel,
                    Text = "－" + "技能修炼值".Lang() + $"：{MouseMagic.Experience1:###0}"
                };
                MagicLabel.Size = new Size((label.DisplayArea.Right + 4 > MagicLabel.Size.Width) ? (label.DisplayArea.Right + 4) : MagicLabel.Size.Width, label.DisplayArea.Bottom);
                label = new DXLabel
                {
                    ...
                    Location = new Point(0, label.DisplayArea.Bottom),
                    Parent = MagicLabel,
                    Text = "2级技能需要等级".Lang() + $"：{MouseMagic.NeedLevel2}"
                };
                MagicLabel.Size = new Size((label.DisplayArea.Right + 4 > MagicLabel.Size.Width) ? (label.DisplayArea.Right + 4) : MagicLabel.Size.Width, label.DisplayArea.Bottom);
                label = new DXLabel
                {
                    ...
                    Location = new Point(0, label.DisplayArea.Bottom),
                    Parent = MagicLabel,
                    Text = "－" + "技能修炼值".Lang() + $"：{MouseMagic.Experience2:###0}"
                };
                MagicLabel.Size = new Size((label.DisplayArea.Right + 4 > MagicLabel.Size.Width) ? (label.DisplayArea.Right + 4) : MagicLabel.Size.Width, label.DisplayArea.Bottom);
                label = new DXLabel
                {
                    ...
                    Location = new Point(0, label.DisplayArea.Bottom),
                    Parent = MagicLabel,
                    Text = "3级技能需要等级".Lang() + $"：{MouseMagic.NeedLevel3}"
                };
                MagicLabel.Size = new Size((label.DisplayArea.Right + 4 > MagicLabel.Size.Width) ? (label.DisplayArea.Right + 4) : MagicLabel.Size.Width, label.DisplayArea.Bottom);
                label = new DXLabel
                {
                    ...
                    Location = new Point(0, label.DisplayArea.Bottom),
                    Parent = MagicLabel,
                    Text = "－" + "技能修炼值".Lang() + $"：{MouseMagic.Experience3:###0}"
                };
                MagicLabel.Size = new Size((label.DisplayArea.Right + 4 > MagicLabel.Size.Width) ? (label.DisplayArea.Right + 4) : MagicLabel.Size.Width, label.DisplayArea.Bottom);
                //    label = new DXLabel
                //  {
                ...
                //dXLabel.Location = new Point(0, GetAndroidCompatiblePosition());
                dXLabel.ForeColour = Color.FromArgb(255, 255, 240);
                dXLabel.Location = new Point(0, label.DisplayArea.Bottom + 4);
                dXLabel.Parent = MagicLabel;
                dXLabel.Text = "说明".Lang() + "：" + MouseMagic.Lang((MagicInfo p) => p.Description);
                int start = 17;
                int cnt = dXLabel.Text.Length / start;
                for (int i = 0; i < cnt; i++)
                {
                    dXLabel.Text = dXLabel.Text.Insert(start, "\r\n");
                    start += 17 + 2;
                }
                label = dXLabel;
                ...
                //label = new DXLabel
                //{
                //    ForeColour = Color.FromArgb(Convert.ToInt32(127.5), 255, 46, 0),
                //    Location = new Point(0, label.DisplayArea.Bottom + 2),
                //    Parent = MagicLabel,
                //    //Text = text
                //};
        ...
        private int GetAndroidCompatiblePosition()
        {
        ...
        private void AddItemLabelDivider()
        {
            ...
            int width = Math.Max(ItemLabel.Size.Width - 8, 120);
            int y = ItemLabel.Size.Height + 2;
            DXLabel line = new DXLabel
            {
                ForeColour = Color.FromArgb(160, 120, 70),
                BackColour = Color.FromArgb(160, 120, 70),
            ...
            ItemLabel.Size = new Size(
                Math.Max(ItemLabel.Size.Width, 4 + width),
                Math.Max(ItemLabel.Size.Height, y + 1 + 3));
        }

        /// <summary>
        /// 套装属性显示标签（默认展示部件与加成，无需按Alt）
        /// </summary>
        private void CreateSetItemInfo(bool showParts = true)
        {
            int level;
            MirClass userClass;
            ...
            foreach (var setDetail in details)
            {
                ...
                label = new DXLabel
                {
                    ForeColour = Color.FromArgb(230, 210, 140),
                    Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                    Parent = ItemLabel,
                    Text = "套装搭配".Lang(),
                };
                ...
                foreach (var activeGroup in activeGroups)
                {
                    ...
                    label = new DXLabel
                    {
                        ...
                        Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                        Parent = ItemLabel,
                        Text = $"    {activeGroup.GroupName} ({activeGroup.GetRequiredNumItem()}/{activeGroup.GetRequiredNumItem()}) {condition}"
                    };

                    ItemLabel.Size = new Size(label.DisplayArea.Right + 4 > ItemLabel.Size.Width ? label.DisplayArea.Right + 4 : ItemLabel.Size.Width,
                        label.DisplayArea.Bottom + 2);

                    // 部件列表：已穿戴绿色 / 未穿戴灰色（默认显示，无需Alt）
                    if (showParts)
                    {
                        foreach (var requiredItem in activeGroup.GetSetGroupItemInfoList())
                        {
                            bool wearing = itemsWearing.Any(x => x != null && x.Index == requiredItem.Index);
                            label = new DXLabel
                            {
                                ForeColour = wearing ? Color.LimeGreen : Color.Gray,
                                Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                                Parent = ItemLabel,
                                Text = wearing ? $"      ● {requiredItem.ItemName}" : $"      ○ {requiredItem.ItemName}"
                            };
                            ItemLabel.Size = new Size(label.DisplayArea.Right + 4 > ItemLabel.Size.Width ? label.DisplayArea.Right + 4 : ItemLabel.Size.Width,
                                label.DisplayArea.Bottom + 2);
                        }
                    ...
                    ItemLabel.Size = new Size(ItemLabel.Size.Width, ItemLabel.Size.Height + 2);
                }
                ...
                foreach (var inactiveGroup in inactiveGroups)
                {
                    ...
                    label = new DXLabel
                    {
                        ...
                        Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                        Parent = ItemLabel,
                        Text = $"    {inactiveGroup.GroupName} ({inactiveGroup.GetNumItemsWearing(itemsWearing)}/{inactiveGroup.GetRequiredNumItem()}) {condition}"
                    };
                ...
                if (!string.IsNullOrEmpty(Globals.SetInfoList.Binding.FirstOrDefault(x => x.SetName == setName)?.SetDescription))
                {
                    label = new DXLabel
                    {
                        ForeColour = Color.FromArgb(200, 200, 160),
                        Location = new Point(4, ItemLabel.DisplayArea.Bottom),
                        Parent = ItemLabel,
...
#if Mobile
            MainPanel.HealthBarLabel.Text = $"H:{Math.Max(0, User.CurrentHP)}";
...
#if Mobile  
            MainPanel.ManaBarLabel.Text = $"M:{Math.Max(0, User.CurrentMP)}";
        ...
        public void UpdateQuestIcons()
        {
            ...
            LeftCentButtonsPanel.AvailableQuestIcon.Visible = QuestBox.AvailableTab.Quests.Count > 0;
            LeftCentButtonsPanel.CompletedQuestIcon.Visible = completed;
