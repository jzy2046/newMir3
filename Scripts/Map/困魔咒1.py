...
def OnEnter(args):            #进入
	...
	Server.Envir.SEnvir.DelayCall("Map.困魔咒1.CloseFuben",300,(map,))   #地图总时间 秒为单位
	map.MapTime = datetime.now()+ timedelta(minutes=5);  #副本地图关卡时间设置
	map.CreateMon(20,23,5,'山洞蝙蝠61',5)
	map.CreateMon(20,23,5,'暗黑战士61',2)
...
def MonsterCount(args):         #怪物总数
	...
	try:
		...
		if map.MonsterCount < 1:
			PlayerSetV(Sender,GV_Taoist_TrapOctagon,2)
			return Sender.TeleportByMapIndex(540,20,23)
		map.MapMsg("地图剩余怪物"+'%d'%map.MonsterCount,MessageType.System)
		Server.Envir.SEnvir.DelayCall("Map.困魔咒1.MonsterCount",3,(map,Sender,))
	except Exception as ex:
		SEnvir.Log("出现如下异常%s"%ex)
		return
	
MapEvent.add_listener(545,"OnEnter",OnEnter)
