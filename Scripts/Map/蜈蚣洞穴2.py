...
def OnEnter(args):            #进入
	...
	Server.Envir.SEnvir.DelayCall("Map.蜈蚣洞穴2.CloseFuben",3600,(map,))   #地图总时间 秒为单位
	#map.MapTime = datetime.now()+ timedelta(minutes=5);  #副本地图关卡时间设置
	map.CreateMon(25,30,10,'跳跳蜂',10)
	map.CreateMon(25,30,10,'蝴蝶虫',10)
	map.CreateMon(25,30,10,'蜈蚣',10)
	map.CreateMon(25,30,10,'沃毒蜈蚣',1)
...
def MonsterCount(args):         #怪物总数
	...
	try:
		...
		Server.Envir.SEnvir.DelayCall("Map.蜈蚣洞穴2.MonsterCount",3,(map,Sender,))
	except Exception as ex:
		SEnvir.Log("出现如下异常%s"%ex)
		return
	
MapEvent.add_listener(531,"OnEnter",OnEnter)		
