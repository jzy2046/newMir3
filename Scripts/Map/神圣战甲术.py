...
def OnEnter(args):            #进入
	...
	Server.Envir.SEnvir.DelayCall("Map.神圣战甲术.CloseFuben",1800,(map,))   #地图总时间 秒为单位
	map.MapTime = datetime.now()+ timedelta(minutes=30);  #副本地图关卡时间设置
	map.CreateMon(25,25,30,'尸王61',20)
...
def MonsterCount(args):         #怪物总数
	...
	try:
		if Sender.CurrentMap != map:
			return
		if (Sender.GetItemCount("起爆石") > 0):
			PlayerSetV(Sender,GV_Taoist_Resilience,3)
			return Sender.NPCCall(31, True)  #调用对应的NPC
		map.MapMsg("地图剩余怪物"+'%d'%map.MonsterCount,MessageType.System)
		Server.Envir.SEnvir.DelayCall("Map.神圣战甲术.MonsterCount",3,(map,Sender,))
	except Exception as ex:
		SEnvir.Log("出现如下异常%s"%ex)
		return

MapEvent.add_listener(528,"OnEnter",OnEnter)
