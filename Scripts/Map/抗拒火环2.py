...
def OnEnter(args):            #进入
	...
	Server.Envir.SEnvir.DelayCall("Map.抗拒火环2.CloseFuben",300,(map,))   #地图总时间 秒为单位
	map.MapTime = datetime.now()+ timedelta(minutes=5);  #副本地图关卡时间设置
	select = random.randint(0,100)
	if select < 20:
		map.CreateMon(40,66,50,'半兽勇士11',10)
		map.CreateMon(66,40,50,'掷斧骷髅',5)
	elif select < 50:
		map.CreateMon(40,66,50,'半兽战士',5)
		map.CreateMon(66,40,50,'毒蜘蛛',10)
	else:
		map.CreateMon(40,66,50,'半兽人',10)
		map.CreateMon(66,40,50,'毒蜘蛛',5)
...
def MonsterCount(args):         #怪物总数
	...
	try:
		...
		if map.MonsterCount < 15:
			PlayerSetV(Sender,GV_Wizard_Repulsion,1)
		...
		Server.Envir.SEnvir.DelayCall("Map.抗拒火环2.MonsterCount",3,(map,Sender,))
	except Exception as ex:
		SEnvir.Log("出现如下异常%s"%ex)
		return

MapEvent.add_listener(514,"OnEnter",OnEnter)		
