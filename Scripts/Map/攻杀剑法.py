...
def OnEnter(args):            #进入
	...
	Server.Envir.SEnvir.DelayCall("Map.攻杀剑法.CloseFuben",180,(map,))   #地图总时间 秒为单位
	map.MapTime = datetime.now()+ timedelta(minutes=3);  #副本地图关卡时间设置
	map.CreateMon(9,12,10,'骷髅',10)
	map.CreateMon(9,12,10,'骷髅战士',1)
...
def MonsterCount(args):         #怪物总数
	...
	try:
		...
		if map.MonsterCount < 1 :
			PlayerSetV(Sender,GV_Warrior_Slaying,3)
			return Sender.NPCCall(112, True)  #调用对应的NPC
		map.MapMsg("地图剩余怪物"+'%d'%map.MonsterCount,MessageType.System)
		Server.Envir.SEnvir.DelayCall("Map.攻杀剑法.MonsterCount",3,(map,Sender,))
	except Exception as ex:
		SEnvir.Log("出现如下异常%s"%ex)
		return
	
MapEvent.add_listener(512,"OnEnter",OnEnter)		
