...
def OnEnter(args):            #进入
	...
	Server.Envir.SEnvir.DelayCall("Map.诱惑之光.CloseFuben",120,(map,))   #地图总时间 秒为单位
	map.MapTime = datetime.now()+ timedelta(minutes=2);  #副本地图关卡时间设置
	map.CreateMon(9,12,5,'半兽人61',15)
...
def MonsterCount(args):         #怪物总数
	...
	try:
		...
		if map.MonsterCount < 1 :
			PlayerSetV(Sender,GV_Wizard_ElectricShock,2)
			return Sender.NPCCall(125, True)  #调用对应的NPC
		map.MapMsg("地图剩余怪物"+'%d'%map.MonsterCount,MessageType.System)
		Server.Envir.SEnvir.DelayCall("Map.诱惑之光.MonsterCount",3,(map,Sender,))
	except Exception as ex:
		SEnvir.Log("出现如下异常%s"%ex)
		return
	
MapEvent.add_listener(515,"OnEnter",OnEnter)
