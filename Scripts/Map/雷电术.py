...
def OnEnter(args):            #进入
	...
	Server.Envir.SEnvir.DelayCall("Map.雷电术.CloseFuben",300,(map,))   #地图总时间 秒为单位
	map.MapTime = datetime.now()+ timedelta(minutes=5);  #副本地图关卡时间设置
	map.CreateMon(25,22,5,'雷电僵尸',1)
	map.CreateMon(25,22,5,'僵尸4',3)
	map.CreateMon(25,22,5,'掷斧骷髅',2)
...
def MonsterCount(args):         #怪物总数
	...
	try:
		...
		if map.MonsterCount < 1 :
			PlayerSetV(Sender,GV_Wizard_ThunderBolt,2)
			return Sender.NPCCall(125, True)  #调用对应的NPC
		map.MapMsg("地图剩余怪物"+'%d'%map.MonsterCount,MessageType.System)
		Server.Envir.SEnvir.DelayCall("Map.雷电术.MonsterCount",3,(map,Sender,))
	except Exception as ex:
		SEnvir.Log("出现如下异常%s"%ex)
		return
	
MapEvent.add_listener(522,"OnEnter",OnEnter)		
