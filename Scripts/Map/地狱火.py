...
def OnEnter(args):            #进入
	...
	Server.Envir.SEnvir.DelayCall("Map.地狱火.CloseFuben",300,(map,))   #地图总时间 秒为单位
	map.MapTime = datetime.now()+ timedelta(minutes=5);  #副本地图关卡时间设置
	map.CreateMon(15,14,5,'火焰沃玛61',5)
	map.CreateMon(15,14,5,'火焰沃玛61',3)
...
def MonsterCount(args):         #怪物总数
	...
	try:
		if Sender.CurrentMap != map:
			return
		if(Sender.GetItemCount("新火镜") > 0):
			PlayerSetV(Sender,GV_Wizard_ScortchedEarth,2)
			return Sender.NPCCall(125, True)  #调用对应的NPC
		map.MapMsg("地图剩余怪物"+'%d'%map.MonsterCount,MessageType.System)
		Server.Envir.SEnvir.DelayCall("Map.地狱火.MonsterCount",3,(map,Sender,))
	except Exception as ex:
		SEnvir.Log("出现如下异常%s"%ex)
		return
	
MapEvent.add_listener(523,"OnEnter",OnEnter)		
