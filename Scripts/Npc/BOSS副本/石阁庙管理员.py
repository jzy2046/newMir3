...
#飞	
		...
		if (Sender.GameGold > 49):   #判断需要的物品数量			
			if(Sender.GetItemCount("牙齿") > 299):                #判断需要的物品数量				
				if(PlayerGetV(Sender,GV_ASGMBOSSFB_COUNT)<3):   #定义个人全局变量
					PlayerSetV(Sender,GV_ASGMBOSSFB_COUNT,PlayerGetV(Sender,GV_ASGMBOSSFB_COUNT)+1)     #赋值个人全局变量为1，代表进入过
					if (Sender.GroupMembers):   
					    Sender.GroupLeave()  #如果判断有组，那么直接退出队伍在进入副本
					map = Server.Envir.SEnvir.CreateMap(1596)                     #开启副本地图  （地图ID）       
					map.CreateMon(66,41,200,'楔蛾',50)           
					map.CreateMon(66,41,200,'红野猪',100) 
					map.CreateMon(66,41,200,'黑野猪',100) 
					map.CreateMon(66,41,200,'蝎蛇',100) 
					map.CreateMon(79,28,10,'超级黑野猪',1)           #副本地图刷新的怪物 （刷怪坐标X,Y,范围,怪物名字或者ID，怪物数量）
					map.MapTime = datetime.now()+ timedelta(minutes=30);  #副本地图关卡时间设置
					Sender.Teleport(map,28,73)	#飞地图ID X坐标 Y坐标  
					SubGameGold(Sender,50)    #扣除物品
					Sender.TakeItem("牙齿",300)                  #扣除物品
					return Dict   #不执行提示框
				else:
		...
		if (Sender.GameGold > 99):   #判断需要的物品数量			
			if(Sender.GetItemCount("结晶") > 19):                #判断需要的物品数量				
				if(PlayerGetV(Sender,GV_BSGMBOSSFB_COUNT)<3):   #定义个人全局变量
					PlayerSetV(Sender,GV_BSGMBOSSFB_COUNT,PlayerGetV(Sender,GV_BSGMBOSSFB_COUNT)+1)     #赋值个人全局变量为1，代表进入过
					if (Sender.GroupMembers):   
					    Sender.GroupLeave()  #如果判断有组，那么直接退出队伍在进入副本
					map = Server.Envir.SEnvir.CreateMap(1597)                     #开启副本地图  （地图ID）       
					map.CreateMon(66,41,200,'楔蛾',50)           
					map.CreateMon(66,41,200,'红野猪',100) 
					map.CreateMon(66,41,200,'黑野猪',100) 
					map.CreateMon(66,41,200,'蝎蛇',100) 
		...
		if (Sender.GameGold > 199):   #判断需要的物品数量			
			if(Sender.GetItemCount("魔魂") > 0):                #判断需要的物品数量				
				if(PlayerGetV(Sender,GV_CSGMBOSSFB_COUNT)<3):   #定义个人全局变量
					PlayerSetV(Sender,GV_CSGMBOSSFB_COUNT,PlayerGetV(Sender,GV_CSGMBOSSFB_COUNT)+1)     #赋值个人全局变量为1，代表进入过
					if (Sender.GroupMembers):   
					    Sender.GroupLeave()  #如果判断有组，那么直接退出队伍在进入副本
					map = Server.Envir.SEnvir.CreateMap(1598)                     #开启副本地图  （地图ID）       
					map.CreateMon(66,41,200,'楔蛾',50)           
					map.CreateMon(66,41,200,'红野猪',100) 
					map.CreateMon(66,41,200,'黑野猪',100) 
					map.CreateMon(66,41,200,'蝎蛇',100) 
...
<font color=\"0xff00ff00\">普通地图：进入条件：300个“牙齿”,50元宝 </font>
