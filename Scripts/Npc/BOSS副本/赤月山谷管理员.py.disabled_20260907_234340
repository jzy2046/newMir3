...
#飞	
		...
		if (Sender.GameGold > 49):   #判断需要的物品数量			
			if(Sender.GetItemCount("指甲") > 299):                #判断需要的物品数量				
				if(PlayerGetV(Sender,GV_ACYBOSSFB_COUNT)<3):   #定义个人全局变量
					PlayerSetV(Sender,GV_ACYBOSSFB_COUNT,PlayerGetV(Sender,GV_ACYBOSSFB_COUNT)+1)     #赋值个人全局变量为1，代表进入过
					if (Sender.GroupMembers):   
					    Sender.GroupLeave()  #如果判断有组，那么直接退出队伍在进入副本
					map = Server.Envir.SEnvir.CreateMap(1602)                     #开启副本地图  （地图ID）        
					map.CreateMon(57,48,100,'血巨人',20)           
					map.CreateMon(57,48,100,'血金刚',20) 
					map.CreateMon(15,18,100,'月魔蜘蛛',20) 
					map.CreateMon(23,18,1,'赤月恶魔',1)           #副本地图刷新的怪物 （刷怪坐标X,Y,范围,怪物名字或者ID，怪物数量）
					map.MapTime = datetime.now()+ timedelta(minutes=30);  #副本地图关卡时间设置
					Sender.Teleport(map,11,24)	#飞地图ID X坐标 Y坐标  
					SubGameGold(Sender,50)    #扣除物品
					Sender.TakeItem("指甲",300)                  #扣除物品
					return Dict   #不执行提示框
				else:
		...
		if (Sender.GameGold > 99):   #判断需要的物品数量			
			if(Sender.GetItemCount("结晶") > 19):                #判断需要的物品数量				
				if(PlayerGetV(Sender,GV_BCYBOSSFB_COUNT)<3):   #定义个人全局变量
					PlayerSetV(Sender,GV_BCYBOSSFB_COUNT,PlayerGetV(Sender,GV_BCYBOSSFB_COUNT)+1)     #赋值个人全局变量为1，代表进入过
					if (Sender.GroupMembers):   
					    Sender.GroupLeave()  #如果判断有组，那么直接退出队伍在进入副本
					map = Server.Envir.SEnvir.CreateMap(1603)                     #开启副本地图  （地图ID）        
					map.CreateMon(57,48,100,'血巨人',20)           
					map.CreateMon(57,48,100,'血金刚',20) 
					map.CreateMon(15,18,100,'月魔蜘蛛',20) 
					map.CreateMon(23,18,1,'赤月恶魔',1)           #副本地图刷新的怪物 （刷怪坐标X,Y,范围,怪物名字或者ID，怪物数量）
		...
		if (Sender.GameGold > 199):   #判断需要的物品数量			
			if(Sender.GetItemCount("魔魂") > 0):                #判断需要的物品数量				
				if(PlayerGetV(Sender,GV_CCYBOSSFB_COUNT)<3):   #定义个人全局变量
					PlayerSetV(Sender,GV_CCYBOSSFB_COUNT,PlayerGetV(Sender,GV_CCYBOSSFB_COUNT)+1)     #赋值个人全局变量为1，代表进入过
					if (Sender.GroupMembers):   
					    Sender.GroupLeave()  #如果判断有组，那么直接退出队伍在进入副本
					map = Server.Envir.SEnvir.CreateMap(1604)                     #开启副本地图  （地图ID）        
					map.CreateMon(57,48,100,'血巨人',30)           
					map.CreateMon(57,48,100,'血金刚',30) 
					map.CreateMon(15,18,100,'月魔蜘蛛',20) 
					map.CreateMon(23,18,1,'赤月恶魔',1)           #副本地图刷新的怪物 （刷怪坐标X,Y,范围,怪物名字或者ID，怪物数量）
...
<font color=\"0xff00ff00\">普通地图：进入条件：300个“指甲”,50元宝 </font>
