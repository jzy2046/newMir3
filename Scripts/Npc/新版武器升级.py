...
#新版武器升级		
		...
		Dict['DialogType']= NPCDialogType.WeaponUpgrade
		say = """我是制炼武器的铁匠，如果武器的级别达到可将武器交给我制炼。
...
#判断武器是否超过MR限制
			...
			userItem = Sender.Equipment[int(EquipmentSlot.Weapon)]
			statAmount = 0
			for stat in userItem.AddedStats:
				if stat.Stat == Stat.MaxDC:
			...
			if statAmount > 999:
				...
				[离开:0]"""
#判断武器是否满级
			elif(Sender.Equipment[int(EquipmentSlot.Weapon)].Level < 17):
				say= """你的武器还没有达到最高等级。
				当你的武器完全升级后请再回来。
...
# 如果职业是 法师 或者 道士 无法给武器增加麻痹几率
		if Sender.Class == Sender.Class.Wizard or Sender.Class == Sender.Class.Taoist:
			say = """你的职业不能为武器添加麻痹几率
