...
def OnClick(args):
	...
	if (Menu == 1):
		if (current_map.MonsterCount == 0 ):    #判断当前角色所在地图是否还有怪物
			Sender.GiveItem("BOSS宝箱",1)  #副本地图刷新的怪物 （刷怪坐标X,Y,范围,怪物名字或者ID，怪物数量）
			map = Server.Envir.SEnvir.CreateMap(1507)                         #开启副本地图（地图ID）
			map.MapTime = datetime.now()+ timedelta(minutes=30);  #副本地图关卡时间设置
			Sender.Teleport(map,31,107)	#飞地图ID X坐标 Y坐标
			map.CreateMon(104,27,1,'超级黑野猪',1)           #副本地图刷新的怪物 （刷怪坐标X,Y,范围,怪物名字或者ID，怪物数量）
...
NpcEvent.add_listener(340,"OnClick",OnClick)
