...
#主菜单
		...
		if (PlayerGetV(Sender,BV_NQ_MAIN)==32):
			if (Sender.GetItemCount('肉汤') < 1 ):
				...
				Sender.GiveItem('入门暗杀匕首',1)
				GiveExperience(Sender,100)
				PlayerSetV(Sender,BV_NQ_MAIN,33)
				Sender.Connection.ReceiveChat("任务日志更新！", MessageType.System)
				say =  """呵呵~ 饿了半天了！多谢你给我带吃的过来啊！
					送你一把我们店里卖的匕首就当是报答你了。挺有用的。
					现在再去铁匠铺找一下肉店金氏，或许有什么让你做的事儿......
					
					[结束:0]"""
		elif (PlayerGetV(Sender,BV_NQ_MAIN)==33):
			say = """肉店不是在 <font color=\"0xff00ff00\"> 446:405 </font> 那儿嘛！
				快点回去看看吧！
				
				[结束:0]"""
		elif(PlayerGetV(Sender,BV_NQ_MAIN)==58):
			say = """夜市商人抢别人家孩子？不会的不会的。。。你再找找吧。
...
NpcEvent.add_listener(61,"OnClick",OnClick)