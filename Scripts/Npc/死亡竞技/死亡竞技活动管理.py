...
def OnClick(args):
	...
	if (Menu == 1):
		if(PlayerGetV(Sender,GV_PLAYER_ZHOUJINGJICHANG)==0):                #定义个人全局变量 
			say = """活动奖励丰厚，要领取奖励前建议你清理包裹剩余7格以上空间在领取，否则奖品遗失概不负责。
		...
		if (Sender.CurrentMap.Players.Count == 1):     #判断地图是否只剩玩家一人
			if Sender.Character.Account.TempAdmin:  # 判断是否是管理员
				return
			...
			if(PlayerGetV(Sender,GV_PLAYER_ZHOUJINGJICHANG)==0):                #定义个人全局变量
				...
				Sender.GiveItemsByStat([{'name':'万年雪霜','bound':True,'count':100,},])    #给100个万年雪霜
				Sender.GiveItemsByStat([{'name':'天下无双·白金','bound':True,'count':1,'Expire':604800,},])    #给天下无双武器一把
				Sender.GiveItemsByStat([{'name':'攻击神水（特）','bound':True,'count':1,},])
				Sender.GiveItemsByStat([{'name':'自然神水（特）','bound':True,'count':1,},])
				Sender.GiveItemsByStat([{'name':'灵魂神水（特）','bound':True,'count':1,},])
				Sender.GiveItemsByStat([{'name':'祝福油','bound':True,'count':10,},])
...
NpcEvent.add_listener(409,"OnClick",OnClick)