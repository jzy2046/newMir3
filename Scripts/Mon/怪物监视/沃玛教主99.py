...
#####################二次触发########################################################
	...
	if MonsterGetTempVDefault(monster,"怪物狂暴1", 1) > 0 and hp_percent < 25:
		...
		if monster.HasMonsterBuff('怪物狂暴1'):
			...
			#给怪物加buff   攻击翻倍 魔法翻倍
			monster.AddBuff('怪物狂暴1', {Stat.MaxDC: 2*monster.Stats[Stat.MaxDC], Stat.MinMC: 2*monster.Stats[Stat.MinMC]}, 1)
			#扣掉次数
...
MonsterEvent.add_listener(100064,"OnProcessAI",OnProcessAI)
MonsterEvent.add_listener(100337,"OnProcessAI",OnProcessAI)
